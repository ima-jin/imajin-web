...
export const productDependencies = pgTable('product_dependencies', {
  ...
  productId: text('product_id').notNull().references(() => products.id, { onDelete: 'cascade' }),
  dependsOnProductId: text('depends_on_product_id').notNull().references(() => products.id, { onDelete: 'cascade' }),
...
export const nftTokens = pgTable('nft_tokens', {
  ...
  variantId: text('variant_id').notNull().references(() => variants.id, { onDelete: 'set null' }),
...
export const portfolioImages = pgTable('portfolio_images', {
  id: serial('id').primaryKey(),
  portfolioItemId: integer('portfolio_item_id').notNull().references(() => portfolioItems.id, { onDelete: 'cascade' }),
