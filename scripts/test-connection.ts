...
async function testConnection() {
  ...
  try {
    ...
    const activeProducts = await db.select().from(products).where(eq(products.devStatus, 5));
    console.log(`✅ Active products (dev_status = 5): ${activeProducts.length}`);

    // Test 3: Get Founder Edition variants
    const founderVariants = await db
      .select()
      .from(variants)
    ...
    for (const variant of founderVariants) {
      console.log(
        `   ${variant.variantValue}: ${variant.soldQuantity}/${variant.maxQuantity} sold (${variant.availableQuantity} available)`
      );
    }
